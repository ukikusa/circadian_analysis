...
def frond_shaping(imgs):
    ...
    for i in range(imgs.shape[0]):
        # まず穴を消す
        img = imgs[i, :, :]
        label, img_label, img_stats, other = cv2.connectiedcomponentsWithStats(img, connectivity=4)
    return 0
...
def frond_traker(imgs, min_size=20):
    ...
    label, imgs_sort[0], stats_tmp, centroids_tmp = cv2.connectedComponentsWithStats(imgs[0], connectivity=4)
    label, imgs_sort[0], stats_tmp, centroids_tmp = trash_frond(label, imgs_sort[0], stats_tmp, centroids_tmp, min_size=min_size)
    ...
    imgs_stats_sort[0, 0:label - 1], imgs_centroids[0, 0:label - 1] = stats_tmp[1:], centroids_tmp[1:]
    ...
    for i in range(1, imgs.shape[0]):
        ...
        # 追跡する画像をラベル付
        label, img, stats, centroids = cv2.connectedComponentsWithStats(imgs[i], connectivity=4)
        label, img, stats, centroids = trash_frond(label, img, stats, centroids, min_size=min_size)
        # 重心間の距離を図る．
...
if __name__ == '__main__':
    ...
    # 白黒でもカラーでもOK
    frond_folder = os.path.join(day, 'edit_raw', 'div_img')
    # frond_folder = day + '/edit_raw/label_img_ito'
    imgs = im.read_imgs(frond_folder)
    imgs_sort, imgs_centroids, imgs_stats_sort, label_out = frond_traker(imgs, min_size=5)
    im.save_imgs(os.path.join(day, 'edit_raw', 'label_imgs_tmp'), imgs_sort)
