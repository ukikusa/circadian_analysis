...
def make_hst_fig(save_file, x, y, min_x=0, max_x=None, min_y=0, max_y=None, max_hist_x=None, max_hist_y=None, bin_hist_x=100, bin_hist_y=100, xticks=False, yticks=False, xticklabels=[], yticklabels=[], pdfpages=False, avg=False, xlabel='', ylabel='', box=False, per=True, title=False):
    ...
    if avg is not False:
        ...
        x_avg = np.arange(max(np.floor(np.min(x)), min_x), min(
            np.ceil(np.max(x)), max_x), avg, dtype=np.float64)  # 平均を取る
        y_avg, y_se = [], []
        for i in x_avg:
            idx = (x < (i + avg)) * (x >= i)
        ...
        ax.errorbar(x_avg + avg * 0.5, y_avg, yerr=y_se,
                    fmt='r.', ecolor='k', elinewidth=0.5)
    #############################
    # box prot
    ############################
    if box is not False:
        ...
        ax_b.boxplot(y_avg, positions=(x_avg + box * 0.5), showmeans=True, meanline=True,
                     whis='range', widths=box, whiskerprops=whiskerprops, showcaps=False)
    ...
    # hisutoglam
    ###################
    ax_x = plt.axes([sc_left, sc_bottom + sc_height +
                     space, sc_width, hst_height])
